...
int bfs(Node start, Node end, int max) {
  queue<Node> search;
  vector<Node> seen;
  search.push(start);
  ...
  while (!search.empty()) {
    ...
    if (current.posx == end.posx && current.posy == end.posy) return current.steps;
    if (current.steps > max) continue;
    //Add left
    Node left;
    ...
    if (!(left.posx < 0)) {
      ...
      if (!searched && ((heightMap[current.posx][current.posy] - heightMap[left.posx][left.posy]) >= -1)) {
    ...
    if (!(right.posx >= heightMap.size())) {
      ...
      if (!searched && ((heightMap[current.posx][current.posy] - heightMap[right.posx][right.posy]) >= -1)) {
    ...
    if (!(up.posy < 0)) {
      ...
      if (!searched && (heightMap[current.posx][current.posy] - heightMap[up.posx][up.posy] >= -1)) {
    ...
    if (!(down.posy >= heightMap[0].length())) {
      ...
      if (!searched && (heightMap[current.posx][current.posy] - heightMap[down.posx][down.posy] >= -1)) {
