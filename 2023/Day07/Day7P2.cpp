...
string cardRanks = "J23456789TQKA";

int getScore(string hand) {
    int longestMatch = 0, uniqueValues = 0, currentMatch = 0, jokers = 0;
    for (int i = 0; i < hand.length(); i++) {
        if (hand[i] == 'J') {
            currentMatch = 0; //Don't count this in our list of matches
            //Jokers pretend to be a different card, don't increment unique values
            jokers++;
            continue;
        }
